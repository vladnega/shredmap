...
- **Markers:** Loaded from `GET /api/bike-parks` via `lib/bike-parks/fetch-bike-parks-for-map.ts`, which validates the JSON and passes points to `replaceBikeParkMarkersOnMap` (`components/map/replace-bike-park-markers.ts`). Map pins use each park's `pinLogoUrl` when available, falling back to `logoUrl` and then an orange circle when no logo is set; logos render as **round** DOM overlays (CSS clipping, works for cross-origin images). Requests respect **abort signals** so rapid navigation or remounts do not apply stale markers.

...
const LEGACY_ACTIONS: Record<string, ActivityType> = {
  CREATE_TEAM: ActivityType.CREATE_ORGANIZATION,
  REMOVE_TEAM_MEMBER: ActivityType.REMOVE_ORGANIZATION_MEMBER,
  INVITE_TEAM_MEMBER: ActivityType.INVITE_ORGANIZATION_MEMBER,
};

function normalizeAction(action: string): ActivityType | string {
...
function formatAction(action: string): string {
  ...
  switch (normalized) {
    ...
    case ActivityType.CREATE_ORGANIZATION:
      return 'You created a workspace';
        ...
        <CardContent>
          {logs.length > 0 ? (
            <ul className="space-y-4">
              {logs.map((log) => {
                const key = normalizeAction(log.action) as string;
            ...
            <div className="flex flex-col items-center justify-center text-center py-12">
              ...
              <p className="text-sm text-gray-500 max-w-sm">
                When you sign in or update your account, entries appear here.
              </p>
            </div>
          )}
        </CardContent>
